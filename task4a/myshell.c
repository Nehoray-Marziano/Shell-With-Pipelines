...
int** createPipes(int numOfPipes);
void releasePipes(int** pipes,int numOfPipes);
int* leftPipe(int** pipes,cmdLine *cmd);
int* rightPipe(int** pipes,cmdLine *cmd);
...
int execute(cmdLine *pCmdline){
    ...
    if(pid_child==0){ /*meaning it is a child*/
        if(pCmdline->outputRedirect!=NULL){
            close(STDOUT_FILENO);
            int fileDes= open(pCmdline->outputRedirect, O_WRONLY);
            dup2(fileDes,STDOUT_FILENO);
            close(fileDes);
        }
        if(pCmdline->inputRedirect!=NULL){
            close(STDIN_FILENO);
            int fileDes=open(pCmdline->inputRedirect,O_RDONLY);
    ...
    else if(pCmdline->blocking==1){ /*parent proccess*/
        waitpid(-1,&status,0);
    }
    return status;
}
...
int** createPipes(int numOfPipes){
    int** pipes=malloc(numOfPipes* sizeof(int*));
    int i;
...
int* leftPipe(int** pipes,cmdLine *cmd){
    if(cmd->idx==0)/*first command-no left*/
        return NULL;
    return pipes[(cmd->idx)-1];    
}

int* rightPipe(int** pipes,cmdLine *cmd){
    if(cmd->next==NULL)
        return NULL;
    return pipes[(cmd->idx)];    
