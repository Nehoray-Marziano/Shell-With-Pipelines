...
{
    cmdLine *newL = (cmdLine *)malloc(sizeof(cmdLine));
    newL->argCount = command->argCount;
    newL->inputRedirect = NULL;
    newL->outputRedirect = NULL;
    newL->blocking = command->blocking;
    newL->idx = 0;
    newL->next = NULL;
    ...
    {
        ((char **)newL->arguments)[i] = strClone(command->arguments[i]);
    }
