...
int execute(cmdLine *pCmdline){
    ...
    if(pid_child==0){ /*meaning it is a child*/
        if(pCmdline->outputRedirect!=NULL){
            close(1);
            int fileDes= fopen(pCmdline->outputRedirect, "w");
            close(fileDes);
        }
        if(pCmdline->inputRedirect!=NULL){
            close(0);
            int fileDes=fopen(pCmdline->inputRedirect,"r");
            close(fileDes);
        }
        if(pCmdline) {
...
cmdNode* add_cmd(cmdNode *originalHead, cmdLine* newCmd){
    ...
    newHead->next=(cmdNode *)originalHead;
...
cmdNode* HistoryDeleteCase(cmdNode *head, cmdLine *cmd) {
    ...
    tempNext=(cmdNode *)curr->next;/*connecting his right and left 'hands'*/
    tempPrev=(cmdNode *)curr->prev;
    tempPrev->next=(cmdNode *)tempNext;
    tempNext->prev=tempPrev;
